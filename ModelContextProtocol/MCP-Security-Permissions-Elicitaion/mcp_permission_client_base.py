...
class MCPPermissionClient:
    ...
    def __init__(self, server_script: str, permissions_file: str = "data/permissions.json"):
        ...
        self.permissions = self.load_permission()

    # implement connection management
    async def connect(self):
        """Connect to MCP server via STDIO. Safe to call multiple times"""
        if self._connected:
            return
    ...
    def load_permission(self) -> dict:
        """Load permissionns from file re return defaults"""
    ...
    async def request_elicitation(self, schema: dict, description: str) -> dict:
        """Request structured user input via elicitaion
        This is conceptual implementation. In porduction, this would trigger a UI dialog ad wait for user response

        Args:
            schema: JSON schema for the required input
            descriptions: Human-readable description of what's needed
