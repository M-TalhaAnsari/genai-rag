...
def get_path(relative_path: str) -> Path:
    ...
    Joins BASE_DIR with the relative path first, then resolves symlinks/dots.
    This ensures the resolved path is always anchored inside BASE_DIR.
    ...
    Raises:
        ValueError: If path attempts to escape BASE_DIR (e.g. ../../etc)
    """
    try:
        # Anchor the path inside BASE_DIR BEFORE resolving.
        # Old code: Path(relative_path).resolve() resolved relative to CWD
        # then .relative_to(BASE_DIR) failed because it returned an absolute
        # path that didn't match BASE_DIR when spaces/special chars were present.
        resolved = (BASE_DIR / relative_path).resolve()
        resolved.relative_to(BASE_DIR)  # security check — raises ValueError if outside
        return resolved
    except ValueError:
        raise ValueError(f"Path '{relative_path}' is outside BASE_DIR")
...
async def list_files_resource() -> dict:
    ...
    """
    try:
        path = BASE_DIR  # Always list BASE_DIR directly — no path resolution needed
