        ...
        return json.dumps(
            {
                ...
                "message": f"No restaurant found matching '{restaurant_name}'. Please check the name and try again.",
                "suggestions": "Try a partial name like 'Iron' or 'Sakura'."
            },
            indent =2
        )
    return json.dumps(
...
@mcp.tool()
def recommend_by_vibe(vibe: str) -> dict:
    """Find restaurant that match a given vibe or atmosphere keyword. Searches both structures vibe tags and raw text description.
    Example of vibe keywords: "moody","sun-drenched","romantic","family-friendly","trendy","cozy","rustic","modern","casual","upscale"
    """
    restaurants = load_restaurant_data()
    vibe_lower = vibe.lower().strip()

    # structured vibe tags in JSON
    structured_matches = []
    ...
    # search the raw text for additional matches
        ...
        return json.dumps(
            {
                ...
                "message": f"No review found for '{restaurant_name}'. Please check the name and try again.",
            },
            indent=2,
        )
    return json.dumps(
