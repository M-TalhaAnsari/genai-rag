"""
The AgentWorkflow class constructs the multiagent system using langGraph stategraph,
ensuring a structured approach to information retrieval and verification
"""
...
class AgentWorkflow:
    ...
    def _check_relevance_step(self, state:AgentState) -> Dict:
        ...
        if classification == "PARTIAL":
            # There is partial coverage , but we can still proceed
            return {"is_relevant":True}
        else:  # Classification == "NO_MATCH"
            return{"is_relevant":False, "draft_answer":"This question isn't related (or there's no data) for your query. Please ask another question relevant to the uploaded document(s)."}
    ...
    def full_pipeline(self, question:str, retriever: EnsembleRetriever):
        try:
            ...
            logger.error(f"Received {len(documents)} relevant documents ( from .invoke)")
